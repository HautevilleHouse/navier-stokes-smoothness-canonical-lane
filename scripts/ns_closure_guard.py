#!/usr/bin/env python3
"""Canonical-lane closure guard for 3D Navier-Stokes workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    kappa_enstrophy, t1 = _entry(consts, "kappa_enstrophy")
    ...
    tau_floor, t5 = _entry(consts, "tau_floor")
    eps_coh, t6 = _entry(consts, "eps_coh")

    n_g1 = t1 and _finite(kappa_enstrophy) and float(kappa_enstrophy) > 0.0
    n_g2 = t2 and _finite(sigma_capture) and float(sigma_capture) > 0.0
    n_g3 = t3 and _finite(kappa_compact) and float(kappa_compact) > 0.0
    n_g4 = t4 and _finite(rho_rigidity) and float(rho_rigidity) > 0.0
    n_g5 = t5 and _finite(tau_floor) and float(tau_floor) > 0.0
    n_g6_base = t6 and _finite(eps_coh) and float(eps_coh) >= 0.0
    n_g6 = n_g6_base and ((abs(float(eps_coh)) < 1e-15) if strict_coh_zero else True)

    margin = None
    if all(_finite(x) for x in (kappa_enstrophy, sigma_capture, kappa_compact, rho_rigidity, tau_floor, eps_coh)):
        margin = min(
            float(kappa_enstrophy),
            float(sigma_capture),
            float(kappa_compact),
            float(rho_rigidity),
            float(tau_floor),
    ...
    n_gm = n_g1 and n_g2 and n_g3 and n_g4 and n_g5 and n_g6 and _finite(margin) and float(margin) > 0.0

    gates = {
        "N_G1": "PASS" if n_g1 else "FAIL",
        "N_G2": "PASS" if n_g2 else "FAIL",
        "N_G3": "PASS" if n_g3 else "FAIL",
        "N_G4": "PASS" if n_g4 else "FAIL",
        "N_G5": "PASS" if n_g5 else "FAIL",
        "N_G6": "PASS" if n_g6 else "FAIL",
        "N_GM": "PASS" if n_gm else "FAIL",
    }
    blockers = {
        "N_G1": [] if n_g1 else ["kappa_enstrophy missing/nonpositive or not theorem-level"],
        "N_G2": [] if n_g2 else ["sigma_capture missing/nonpositive or not theorem-level"],
        "N_G3": [] if n_g3 else ["kappa_compact missing/nonpositive or not theorem-level"],
        "N_G4": [] if n_g4 else ["rho_rigidity missing/nonpositive or not theorem-level"],
        "N_G5": [] if n_g5 else ["tau_floor missing/nonpositive or not theorem-level"],
        "N_G6": [] if n_g6 else ["eps_coh missing/invalid or strict-zero target failed"],
        "N_GM": [] if n_gm else ["strict margin <= 0 or upstream gates failed"],
    }
    normalized_gates = {
        "G1": gates["N_G1"],
        "G2": gates["N_G2"],
    ...
    report = {
        ...
        "inputs": {
            "kappa_enstrophy": kappa_enstrophy,
